-- Paints the Premade Groups Filter dialog and its panels in the user's current
-- EllesmereUI theme, through the EllesmereUI skinning API:
-- https://github.com/EllesmereGaming/EllesmereUI/blob/main/SKINNING_API.md
--
-- Only the two frames Premade Groups Filter exports are used:
--   PremadeGroupsFilter.DialogFrame
--   PremadeGroupsFilter.PopupMenuFrame

-- The name this skin registers under. EllesmereUI shows it in its options, under
-- Blizz UI Enhanced > Blizzard Window Skins > Third-Party Addons, so it names the
-- addon being skinned rather than this one.
local SKIN_NAME = "PremadeGroupsFilter"

-- Font size Premade Groups Filter uses for its text boxes (PGF.C.FONTSIZE_TEXTBOX).
local FONTSIZE_TEXTBOX = 12
...
--- Reapplies the skin to the shared popup menu after the click that opened it.
--- Premade Groups Filter rebuilds the menu on every open, creating entries on demand
--- and repainting the border of the frame, both of which undo the skin. The hook runs
--- after the OnClick handler that opens the menu, so by then it has been built and
--- shown. Every menu is opened by one of these buttons, so this covers all of them.
local function HookPopupMenu(S, button)
...
local function ApplyTextBoxFont(S, editBox)
    ...
    editBox:SetFont(fontPath, FONTSIZE_TEXTBOX, fontFlag)
end
...
local function SkinPanels(S, dialog)
    local skinned = {}
    for _, panel in pairs(dialog.panels or {}) do
...
if EllesmereUI and EllesmereUI.RegisterSkin then
    EllesmereUI.RegisterSkin(SKIN_NAME, ApplySkin)
end
